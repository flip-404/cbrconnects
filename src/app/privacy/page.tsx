...
type PrivacyClause = { title: string; content?: string; subclauses?: string[] }

export default function KCanberraTerms() {
  const [activeTab, setActiveTab] = useState('terms')

  const termsData: PrivacyClause[] = [
    {
      title: '제1조 목적',
      content:
        '본 이용약관은 "케이캔버라"(이하 "사이트")의 서비스의 이용조건과 운영에 관한 제반 사항 규정을 목적으로 합니다.',
    },
    {
      title: '제2조 용어의 정의',
      content: '본 약관에서 사용되는 주요한 용어의 정의는 다음과 같습니다.',
      subclauses: [
  ...
  const moreTermsData = [
    ...
    { title: '부칙', content: '이 약관은 <사이트 개설일>부터 시행합니다.' },
  ]
  ...
  const allTermsData = [...termsData, ...moreTermsData]

  const renderClause = (clause: PrivacyClause) => {
    return (
      <div key={clause.title} className="mb-6 p-4 bg-gray-50 rounded-lg">
        <h3 className="font-bold text-blue-600 mb-3">{clause.title}</h3>
        {clause.content && <p className="text-gray-700 mb-3 leading-relaxed">{clause.content}</p>}
        {clause.subclauses && (
          <div className="space-y-2">
            {clause.subclauses.map((subclause, idx) => (
              // eslint-disable-next-line react/no-array-index-key
              <p key={idx} className="text-gray-600 leading-relaxed pl-4">
                {subclause}
              </p>
            ))}
          </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white">
      <h1 className="text-3xl font-bold text-center text-blue-600 mb-8 pb-4 border-b-2 border-gray-200">
        케이캔버라 이용약관 및 개인정보처리방침
      </h1>
      ...
      <div className="min-h-screen">
        {activeTab === 'terms' && (
          <div className="space-y-4">{allTermsData.map((clause) => renderClause(clause))}</div>
        )}

        {activeTab === 'privacy' && (
          <div className="bg-blue-50 p-6 rounded-lg">
            <h2 className="text-2xl font-bold text-blue-600 mb-6">개인정보처리방침</h2>
            <div className="space-y-4">{privacyData.map((clause) => renderClause(clause))}</div>
          </div>
        )}
      </div>
    </div>
